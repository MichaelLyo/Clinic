...
scp -r ./Clinic/ ./MyClinic/ ./db.sqlite3 ./manage.py ./requirements.txt ubuntu@118.25.6.157:myproject/clinic
#ssh ubuntu@212.64.32.162 "sudo service nginx restart ; uwsgi --reload /home/ubuntu/LoanCredit/uwsgi/uwsgi.pid"
ssh ubuntu@118.25.6.157 "python myproject/clinic/manage.py runserver 0.0.0.0:88 --insecure"

#sed -ig 's/DEBUG = False/DEBUG = True/'  ./LoanCredit/settings.py
#rm ./LoanCredit/settings.pyg

#在运行时强制Django处理静态文件：(不需要配置uwsgi和nginx)
#python manage.py runserver 0.0.0.0:88 --insecure